    ...
    <div>
      ...
      <div className="mt-8">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { tab: 'content', label: 'Edit Site Content', desc: 'Hero text, about section, footer' },
            { tab: 'menu', label: 'Manage Menu', desc: 'Sectors, items, pricing, bundles' },
            { tab: 'gallery', label: 'Manage Gallery', desc: 'Upload and reorder images' },
            { tab: 'reviews', label: 'Edit Reviews', desc: 'Customer testimonials' },
          ].map((link) => (
            <button
              key={link.tab}
              onClick={() => onNavigate(link.tab)}
              className="border border-gray-200 bg-white p-4 text-left transition-colors hover:border-orange-300 hover:bg-orange-50"
            >
